...
tmux send-keys -t 1 "watch 'az containerapp revision list -g $RESOURCEGROUP -n httpapi --query [].{Revision:name,Replicas:replicas,Active:active,Created:createdTime} -o table' 2\>\/dev\/null" C-m

tmux send-keys -t 2 "watch 'curl -s $DATAURL'" C-m
